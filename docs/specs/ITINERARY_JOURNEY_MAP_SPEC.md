...
| Trip facts | [SPAIN_2026_MASTER_TRIP_NOTE.md](../source/SPAIN_2026_MASTER_TRIP_NOTE.md) |
...
Source of truth for trip content: `docs/source/SPAIN_2026_MASTER_TRIP_NOTE.md` ("the master note"). Where the master note and `src/data/mockData.ts` conflict, the master note wins — conflicts are documented, not silently resolved.
...
| 3 | Menorca rental car | Absent from `mockLogistics` entirely | DoYouSpain **DYS-207641506**, Autos Menorca NR, pickup Jul 29 5:00 PM, **voucher return Aug 1 8:00 AM — impossible against the 7:50 AM flight** | Add as a `BookingReference` + `UnresolvedTask` with `priority: "critical"`. This is the single highest-stakes unresolved item in the trip and today it doesn't exist in the app at all. |
| 4 | Barcelona/Costa Brava car | `log-5`/`log-6`: company "Avis," ref `AVIS-48821`, pickup "Avis Blanes Station," return Aug 2 4:00 PM | DoYouSpain **DYS-208785865**, Drivalia Rent A Car, pickup Barcelona Airport Aug 1 9:00 AM, **voucher return Aug 3 12:00 PM, desired return before 9:00 AM Aug 3** | Replace with correct provider/ref/dates. The return date is off by a full day (Aug 2 vs. Aug 3), and the early-return gap (12:00 PM voucher vs. ~8:30–9:00 AM desired) is itself an `UnresolvedTask`. |
...
| 6 | Menorca stay | "Hotel Rural Morvedra Nou," check-in `2026-07-28` 2:00 PM | **Airbnb/condo, host Scott**, Carrer de la Mediterrània, Cala en Porter, check-in after 4:00 PM **July 29** | Correct the property type, name, and date. Check-in on Jul 28 is impossible — the traveler is still mid-flight that day. |
...
- **Must do:** Check in for DL128; board with medications/valuables/one fresh outfit in personal item
...
- **Overnight base:** Cala en Porter Airbnb (host: Scott) — **confirmed** (correcting §0 conflict #6)
...
- **Booking dependencies:** `DL128` confirmed · `FR7509` confirmed (ref W4LNUT) · Menorca car `DYS-207641506` confirmed-but-conflicted (see day-3) · Cala en Porter stay confirmed
...
- **Booking dependencies:** `DYS-207641506` (Menorca car) — confirmed booking, **unresolved logistics**
- **Energy mode:** `soft`
...
- **Booking dependencies:** `DYS-208785865` (Drivalia, Barcelona/Costa Brava car) confirmed · Gran Hotel Reymar confirmed
...
- **Booking dependencies:** `DYS-208785865` return — unresolved against voucher terms
- **Energy mode:** `medium`
