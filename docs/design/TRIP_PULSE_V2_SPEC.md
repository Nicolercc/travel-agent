...
> **As shipped (RFC-001, E5):** Trip Pulse now answers "what needs me?" first. The hierarchy is: header with the
> demo date → **Needs you** (top three issues, each resolvable in place: Add confirmation / Mark done / Open day) →
> journey ribbon + selected-day preview (verdict and top reason) → a collapsed **Readiness** summary derived from the
> same issues. Removed: RouteVisualization (decorative and wrong), Quick Access, and "next best actions" as a separate
> block. The sections below record the original Phase 1.3 intent.
...
Refine Trip Pulse hierarchy and emotional experience under the [Soft Coastal Visual System](SOFT_COASTAL_VISUAL_SYSTEM.md). This is **not a recolor** — it is a composition and priority pass.
